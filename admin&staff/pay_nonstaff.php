...
<nav class="navbar navbar-inverse navbar-fixed-top">
  ...
  <section id="breadcrumb">
    <div class="container">
      <ol class="breadcrumb">
        <li class="active"><a href="admin.php">DASHBOARD</a> /  NON-STAFF PAYMENT</li>
      </ol>
    </div>
  </section><!-- Section 1 -->
        ...
        <div class="col-md-9">
          <div class="panel panel-default">
            <div class="panel-heading" id="main-bg-color">
              <h3 class="panel-title"><center><span class="icon-money"></span> NON-STAFF PAYMENT FORM</center></h3>
            </div>
            <div class="panel-body">
                <?php 
                    if(isset($_POST['save_data'])){
                        $nonstaff_id = $_POST['nonstaff_id'];
                        $nonstaff_salary = $_POST['nonstaff_salary'];
                        $date_paid = $_POST['date_paid'];
                        $paid_salary = $_POST['paid_salary'];

                        $querynonst = mysql_query("INSERT INTO `nonstaff_payment`(`nonstaff_id`, `salary`, `date_paid`, `paid_salary`) VALUES ('$nonstaff_id','$nonstaff_salary','$date_paid','$paid_salary')");

                        if($querynonst){
                            echo "<div class='alert alert-success'><center>Successfully saved</center></div>";
                ...
                ?>
                <form role="form" method="post" action="" class="row" id="block-validate">

                    <div class="col-md-6">
                        <div class="form-group">
                        <label for="text" class="control-label">Choose Nonstaff name:</label>
                        <select class="form-control" name="nonstaff_id" required>
                            <option disabled selected>Select Nonstaff Name</option>
                            <?php
                                $seltr = mysql_query("SELECT * FROM non_staff");
                                while ($fet = mysql_fetch_array($seltr)) {
                                    echo "<option value='".$fet['id']."'>".$fet['firstname']." ".$fet['lastname']."</option>";
                                } 
                            ?>
                        </select>
                        </div>
                        ...
                        <select class="form-control" name="nonstaff_salary" required>
                            <option disabled selected>Select Nonstaff Salary</option>
                            <?php
                                $seltr = mysql_query("SELECT * FROM non_staff");
                                while ($fet = mysql_fetch_array($seltr)) {
                                    echo "<option value='".$fet['salary']."'>".$fet['salary']." (".$fet['firstname']." ".$fet['lastname'].")</option>";
                                } 
                            ?>
                        </select>
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="date_paid" id="date" required>
                        </div>
                        
                        <div class="form-group">
                            <label for="pwd">Paid Salary:</label>
                            <input type="number" name="paid_salary" class="form-control" required>
                        </div>
                        
                        
                        <button type="submit" name="save_data" class="btn btn-primary btn-block"><span class="icon-save"></span> SAVE DATA</button>
                    </div>
                </form>
            </div>
          </div>
