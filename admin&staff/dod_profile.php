...
$dod_sel = mysql_query("SELECT * FROM dod WHERE email='$email'")or die("mysql_error".mysql_error());
$fet_dod = mysql_fetch_array($dod_sel);
...
<nav class="navbar navbar-inverse navbar-fixed-top">
  ...
  <section id="breadcrumb">
    <div class="container">
      <ol class="breadcrumb">
        <li class="active"><a href="dod.php">HOME</a> /  DEAN OF DISCIPLINE'S PROFILE</li>
      </ol>
    </div>
  </section><!-- Section 1 -->
        ...
        <div class="col-md-9">
          <div class="panel panel-default">
            <div class="panel-heading" id="main-bg-color">
              <h3 class="panel-title"><center><span class="icon-user"></span> DEAN OF DISCIPLINE'S PROFILE</center></h3>
            </div>
            <div class="panel-body">
              <?php
               if (isset($_GET['save_update'])) {
                 ...
                 $a_u_query = mysql_query("UPDATE `dod` SET `firstname`='$Newfirstname',`lastname`='$Newlastname',`email`='$email',`gender`='$gender',`age`='$age',`province`='$Newprovince',`district`='$Newdistrict',`sector`='$Newsector',`phone`='$phone',`other_phone`='$other_phone' WHERE id='$id'");
                 if($a_u_query){
              ...
              ?>
              <?php
                $query = mysql_query("SELECT * FROM dod WHERE email='$email'");
                $row = mysql_fetch_array($query);
              ?>
              <form class="form-inline row" action="dod_profile.php" method="get" role="form">
                ...
                <div class="form-group">
                  ...
                  <input type="text" class="form-control" name="other_phone" value="<?php echo $row[other_phone]; ?>" required>
                </div>
                ...
                <?php
                 if (isset($_POST['update'])) {
                   ...
                   $check_pass = mysql_query("SELECT * FROM users_tb WHERE email='$email'");
                   $p_row = mysql_fetch_array($check_pass);
                   if($old_password == $p_row['password']){
                     if(strlen($Newpassword < 6)){
                       if($Re_password == $Newpassword){
                         if($n_query = mysql_query("UPDATE `users_tb` SET `password`='$Newpassword' WHERE email='$email'")){
