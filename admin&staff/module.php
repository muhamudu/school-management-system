...
if(isset($_POST['class_id2']))
{
    ...
    $query1=mysql_query("select * from module_tb where level='".$fetch['class']."' and teacher_email='".$fetchm['email']."' ")or die(mysql_error());
...
if(isset($_POST['subject']))
{
    ...
    $query=mysql_query("select * from module_tb where competence_title='$subject'")or die(mysql_error());
    while($fetch=mysql_fetch_array($query)){
        echo "<option value='".$fetch['id']."'>".$fetch['credit']."/".$fetch['hour']."</option>";
    }
}
if(isset($_POST['subject3']))
{
    ...
    $query_tech=mysql_query("select * from teacher_tb where id='".$_POST['tech']."'")or die(mysql_error());
    $fetchm=mysql_fetch_array($query_tech);
    $query=mysql_query("select * from module_tb where competence_title='$subject' and teacher_email='".$fetchm['email']."' ")or die(mysql_error());
    $fetch=mysql_fetch_array($query);
    echo $fetch['id'];
}
if(isset($_POST['subject2']))
{
    ...
    $query=mysql_query("select * from module_tb where competence_title='$subject'")or die(mysql_error());
    while($fetch=mysql_fetch_array($query)){
        echo "<option value='".$fetch['id']."'>".$fetch['hour']."</option>";
    }
